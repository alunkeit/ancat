...
public class UIViewerComponent<V, E> extends JApplet
{
  ...
  public UIViewerComponent( Graph<V, E> graph_ )
  {
    this.graph = graph_;

    graphLayout = new FRLayout<V, E>( graph );
    ((FRLayout) graphLayout).setMaxIterations( 1000 );

    Dimension preferredSize = new Dimension( 1600, 1600 );

    final VisualizationModel<V, E> visualizationModel =
        new DefaultVisualizationModel<V, E>( graphLayout, preferredSize );
    vv = new VisualizationViewer<V, E>( visualizationModel, preferredSize );

    PickedState<V> ps = vv.getPickedVertexState();
    PickedState<E> pes = vv.getPickedEdgeState();
    vv.getRenderContext().setVertexFillPaintTransformer(
        new PickableVertexPaintTransformer<V>( ps, Color.red, Color.yellow ) );
    vv.getRenderContext().setEdgeDrawPaintTransformer(
        new PickableEdgePaintTransformer<E>( pes, Color.black, Color.cyan ) );
    ...
    positionPanel.setBorder( BorderFactory
        .createTitledBorder( "Label Position" ) );
